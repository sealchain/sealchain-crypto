...
import           Seal.Crypto.Wallet
import           Seal.Crypto.Encoding.Seed
import           Seal.Crypto.Encoding.BIP39
import           Crypto.Encoding.BIP39.English (english)
import qualified Seal.Crypto.Praos.VRF as VRF
import           Seal.Internal.Compat (fromRight)
...
type GoldenPaperWallet n
    = "seal" :> "crypto" :> PathParameter "scramble" n
...
type HDWallet n
    = "seal" :> "crypto" :> "wallet" :> PathParameter "BIP39-" n
...
goldenHDWallet = group $ do
    summary "This test vectors uses the `Seal.Crypto.Wallet` primitives to produce extended\n\
