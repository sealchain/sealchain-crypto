     ...
     encrypted_key *encrypted_key)
{
	...
	seal_crypto_ed25519_publickey(secret_key, pub_key);
     ...
     encrypted_key *encrypted_key)
{
	...
	if (seal_crypto_ed25519_extend(seed, secret_key))
		return 1;
     ...
     ed25519_signature signature)
{
	...
	seal_crypto_ed25519_publickey(priv_key, pub_key);
	seal_crypto_ed25519_sign(data, data_len, encrypted_key->cc, CHAIN_CODE_SIZE, priv_key, pub_key, signature);
...
static void add_left(ed25519_secret_key res_key, uint8_t *z, ed25519_secret_key priv_key, derivation_scheme_mode mode)
{
	...
	case DERIVATION_V1:
		...
		/* Kl = 8*Zl + parent(K)l */
		seal_crypto_ed25519_scalar_add(zl8, priv_key, res_key);
		break;
	case DERIVATION_V2:
		/* get 8 * Zl */
...
static void add_left_public(uint8_t *out, uint8_t *z, uint8_t *in, derivation_scheme_mode mode)
{
	...
	/* Kl = 8*Zl*B + Al */
	seal_crypto_ed25519_publickey(zl8, pub_zl8);
	seal_crypto_ed25519_point_add(pub_zl8, in, out);
}
